...
pub(crate) fn init() {
    ...
    if log::set_boxed_logger(Box::new(ImpressyLogger { inner: logger })).is_ok() {
        log::set_max_level(max_level);
    }
}

struct ImpressyLogger {
    inner: Logger,
}

impl Log for ImpressyLogger {
            ...
            DxgiDebugProbeLog::ReplaceWithInfo => {
                self.inner.log(
                    &Record::builder()
                        ...
                        .target("impressy_app::startup")
                        .module_path(Some("impressy_app::logging"))
                        .build(),
                );
                return;
            }
