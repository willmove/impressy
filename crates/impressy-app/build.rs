...
fn main() {
    ...
    let icon = manifest_dir.join("assets/icons/impressy.ico");
    println!("cargo:rerun-if-changed={}", icon.display());

    assert!(
        icon.is_file(),
        "Impressy Windows icon is missing; regenerate the application icons"
    );
    ...
    let icon = icon
        .to_str()
        .expect("Impressy Windows icon path must be valid UTF-8");
    let mut resource = winresource::WindowsResource::new();
    resource
        .set_icon(icon)
        .set("ProductName", "Impressy")
        .set("FileDescription", "Impressy local image processing toolbox")
        .set("OriginalFilename", "impressy.exe");
    resource
        .compile()
        .expect("failed to embed the Impressy Windows icon");
}
