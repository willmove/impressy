...
use impressy_core::config::{self, AppConfig, GenerationHistoryRecord, Language};
use impressy_core::format::{OutputFormat, PngCompression, Quality};
    ...
    (0u8..3, 1u8..=100u8, 0u8..3, any::<bool>(), any::<bool>()).prop_map(
        |(fmt, q, png, lang, has_dir)| AppConfig {
            ...
            last_output_dir: if has_dir {
                Some(PathBuf::from("/tmp/impressy-out"))
            } else {
                None
            },
...
proptest! {
    // Feature: impressy, Property 3: Configuration round-trip preserves data
    #[test]
    ...
    // Feature: impressy, Property 23: Configuration save-reload is idempotent
    #[test]
...
// Feature: impressy, Property 26: Invalid TOML returns error without crash
#[test]
