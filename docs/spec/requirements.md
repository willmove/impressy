...
| `rastery-app` 八个 v1 功能页 | 已实现；待真机验收 | Windows 11 build 26200 的主流程矩阵已验证 release 渲染、中英运行时切换、原生 picker 的选择 / 取消 / 恢复，以及编辑、拼图、批处理四模式、切图、二维码、EXIF、美化、GIF 的导入、参数切换与导出；批量水印覆盖文字 / 图片和右下角 / 平铺。完整清单中的拖放、跨应用剪贴板、错误分类、部分参数边界与系统应用播放，以及 Windows 10 和其他桌面仍待验收 |
