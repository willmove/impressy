...
（Mesa 的 Vulkan ICD 含 `lvp`（Lavapipe 软件光栅化器），理论上可用 Xvfb + Lavapipe 做离屏冒烟测试。但软渲染下 NFR-03「冷启动 ≤ 1.5 秒」不可测量，自定义 Element 的交互手感也测不出来。）
...
> - 仍然验不了的：渲染正确性、自定义 Element 的交互手感、NFR-03 冷启动耗时。
...
按本 ADR 的「core 优先、按可验证性分层」原则，v1 两个 crate 的当前状态：
